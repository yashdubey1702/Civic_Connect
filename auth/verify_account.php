...
// Loads the newest password reset request for an email.
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (strlen($otp) !== 6) {
        ...
        if (hash_equals((string)$row['otp'], $otp) && (int)$row['remaining_seconds'] > 0) {
            ...
            header("Location: /town_issues/auth/reset_password.php");
            exit;
...
<header class="app-header py-3 px-4 px-md-5 d-flex justify-content-between align-items-center">
  ...
  <div class="d-none d-sm-flex gap-3 small">
    <a href="/town_issues/auth/login.php" class="fs-4 text-white fw-semibold text-decoration-none">Log In</a>
    <a href="/town_issues/public/index.html" class="fs-4 text-white text-decoration-none">Home Page</a>
  </div>
</header>
  ...
  <div class="text-center mb-3">
    ...
    <a href="/town_issues/auth/forget_password.php" class="text-primary small fw-semibold text-decoration-none">
  ...
  <form method="POST" id="otp-form" novalidate>
    ...
    <div class="text-center small text-muted mb-4">
      ...
      <a href="/town_issues/auth/resend_otp.php" id="resend-btn" class="fw-bold text-primary text-decoration-none" style="pointer-events:none; opacity:0.5;">
      Resend
      </a>
    </div>
  ...
  <script>
    ...
    // Stores one OTP digit and advances the input focus.
    ...
    // Updates the OTP countdown timer.
